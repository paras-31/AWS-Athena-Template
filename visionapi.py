...
# Define query parameters and headers for the request
query_params = {}
headers = {
...
# Filter the JSON data and count risk levels
if isinstance(response_data, dict) and 'scanResults' in response_data:
    ...
    exclude_rule_ids = {'KMS-006', 'S3-016'}
    
    for result in response_data['scanResults']:
        if result['status'] == 'FAILURE' and result['ruleId'] not in exclude_rule_ids:
